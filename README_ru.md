...
Опции:
  ...
  -a, --api <SOCKET_ADDR> Добавлять/Удалять пиры, используя admin API. Опционально можно указать URI для admin socket (приоритет: аргумент CLI → AdminListen → по-умолчанию для платформы) 
  ...
  -d, --dedup [<PROTO>...] Дедупликация пиров по адресу. Опционально принимает разделенный запятыми список протоколов в порядке их приоритета.
...
Options:
  ...
  -d, --dedup [<PROTO>...] Дедупликация пиров по адресу. Опционально принимает разделенный запятыми список протоколов в порядке их 
  -h, --help                    Print help
